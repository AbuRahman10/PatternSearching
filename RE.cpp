//
// Created by abobaker on 28/03/2021.
//
...
{
    renfa["transitions"].push_back({{"from", to_string(from)},{"to", to_string(to)},{"input", input}});
}
...
void RE::findStartEnd(block *a, int *startPoint, int *endPoint)
{
    ...
    else if (all_of(a->expression.begin(), a->expression.end(), [](char i) { return (i != '(' && i != ')'); }))
    {
        *startPoint = 0;
...
void RE::findExpressions(int startPoint, int endPoint, string *expression1, string *expression2, string *operation,block a)
{
    if (startPoint == 0)
    {
...
void RE::buildBlock(block *x, block *y, block const &a, string const &expression1, string const &expression2,string const &operation)
{
    if (operation == "*")
    {
...
void RE::buildStates(int end)
{
    for (int i = 1; i < end; i++)
    {
        renfa["states"].push_back({{"name", to_string(i)},{"starting", false},{"accepting", false}});
    }
}
...
ENFA RE::toENFA()
{
    ...
    renfa =
    {
        ...
        {"transitions", {}},
        {"alphabet",    {}},
        {"states",      {{{"name", to_string(0)}, {"starting", true}, {"accepting", false}},{{"name", to_string(opers)}, {"starting", false}, {"accepting", true}}}}
    };
    block start = {0, opers, regex};
    buildStates(opers);
